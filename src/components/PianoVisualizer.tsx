    ...
    <div className="w-full bg-slate-950/60 border border-slate-800/80 rounded-2xl p-2.5 sm:p-3 shadow-inner">
      ...
      <div className="relative h-14 sm:h-18 w-full select-none flex">
        ...
        {whiteKeys.map((k) => {
          ...
          let bgClass = 'bg-slate-200 border-slate-400 hover:bg-slate-100';
          if (isPlaying) {
            bgClass = 'bg-indigo-500 border-indigo-400 shadow-md shadow-indigo-500/50 ring-2 ring-indigo-400';
          } else if (isRevealed) {
          ...
          return (
            <div
              key={k.midi}
              className={`flex-1 border-r last:border-r-0 rounded-b-md transition-colors duration-75 relative ${bgClass}`}
            >
              {isRoot && (
          ...
          {whiteKeys.map((wk, idx) => {
            ...
            return (
              <div key={wk.midi} className="flex-1 relative h-full">
                {hasBlack && (
                  <div
                    className={`absolute top-0 right-[-28%] w-[56%] h-[62%] rounded-b-md z-10 border transition-colors duration-75 ${
                      activeMidis.includes(nextMidi)
                        ? 'bg-indigo-500 border-indigo-300 shadow-md shadow-indigo-500/50 ring-2 ring-indigo-400'
